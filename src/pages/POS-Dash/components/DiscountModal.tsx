        ...
        <div className="fixed inset-0 bg-black/60 z-[100] flex items-center justify-center p-4 backdrop-blur-sm animate-in fade-in duration-200">
            <div className="bg-white rounded-[24px] w-full max-w-sm shadow-2xl overflow-hidden flex flex-col">
                <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                    <div className="flex flex-col">
                        <h2 className="text-[20px] font-extrabold text-gray-900 tracking-tight flex items-center gap-2">
                            <Tag size={20} className="text-[#ea580c]" /> Add Discount
                        </h2>
                    </div>
                    <button
                        onClick={onClose}
                ...
                <div className="p-6 space-y-5">
                    <div className="flex bg-gray-100/80 p-1.5 rounded-xl border border-gray-100">
                        <button
                            ...
                            className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-sm font-bold transition-all ${type === 'percent'
                        ...
                        <button
                            ...
                            className={`flex-1 flex items-center justify-center gap-1.5 py-2.5 rounded-lg text-sm font-bold transition-all ${type === 'flat'
                                    ? 'bg-white shadow-sm text-gray-900 border border-gray-200'
                                    : 'text-gray-500 hover:text-gray-700'
                                }`}
                        >
                    ...
                    <div className="relative">
                        <div className="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none text-gray-400">
                            {type === 'percent' ? <Percent size={20} strokeWidth={2.5} /> : <IndianRupee size={20} strokeWidth={2.5} />}
                        </div>
                        <input
                            type="number"
                            value={value}
                            onChange={(e) => setValue(e.target.value)}
                            className={`w-full bg-white border-2 rounded-xl pl-11 pr-4 py-4 text-[24px] font-bold focus:outline-none transition-all ${isInvalid
                    ...
                    {isInvalid && (
                        <p className="text-red-500 text-[13px] font-bold -mt-2">
                    ...
                    <div className="bg-gray-50 rounded-2xl p-4 border border-gray-100 flex items-center justify-between shadow-sm">
                        <span className="font-semibold text-gray-500 text-[14px]">Updated Subtotal</span>
                        <div className="flex flex-col items-end">
                            <span className="text-[13px] text-gray-400 line-through"><IndianRupee size={10} className="inline mr-0.5" />{subtotal.toFixed(2)}</span>
                            <span className="text-[22px] font-black text-green-600 flex items-center -tracking-wide">
                ...
                <div className="p-6 border-t border-gray-100 bg-white flex gap-3">
                    <button
                        onClick={handleClear}
                        className="flex-1 py-4 bg-white border-2 border-gray-200 text-gray-600 font-bold rounded-xl hover:bg-gray-50 hover:border-gray-300 hover:text-gray-800 transition-colors"
                    >
                        Clear
                    </button>
                    <button
                        ...
                        className={`flex-[2] flex items-center justify-center gap-2 font-bold py-4 rounded-xl transition-all shadow-sm ${isInvalid || (parsedValue === 0 && currentValue === 0)
