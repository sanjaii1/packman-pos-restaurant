            ...
            <div className="bg-white rounded-[24px] w-full max-w-lg shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
                {step === 'split' && (
                    <>
                        <div className="px-6 py-5 border-b border-gray-100 flex items-center justify-between bg-gray-50/50">
                            <div>
                                <h2 className="text-[20px] font-extrabold text-gray-900 tracking-tight">Split Bill</h2>
                                <p className="text-[13px] text-gray-500 font-medium mt-0.5">Combine multiple payment methods</p>
                            </div>
                            <button
                                onClick={onClose}
                                className="w-10 h-10 flex items-center justify-center bg-white border border-gray-200 text-gray-400 hover:text-gray-600 hover:bg-gray-50 rounded-xl transition-colors shadow-sm"
                            >
                        ...
                        <div className="flex-1 overflow-y-auto p-6 space-y-6">
                            <div className="bg-gray-50 rounded-2xl p-5 border border-gray-100 flex items-center justify-between shadow-sm">
                                <span className="font-bold text-gray-500 text-[15px]">Total Amount</span>
                                <span className="text-[28px] font-black text-gray-900 flex items-center -tracking-wide">
                            ...
                            <div className="space-y-3">
                                {splits.map((split) => (
                                    <div key={split.id} className="flex gap-3 items-center bg-white border border-gray-200 p-3 rounded-2xl shadow-sm transition-all focus-within:ring-2 focus-within:ring-[#ea580c]/20 focus-within:border-[#ea580c]/50">
                                        <div className="flex bg-gray-100/80 rounded-xl p-1 gap-1 flex-shrink-0">
                                            {(['cash', 'card', 'qr'] as PaymentMethod[]).map(method => (
                                                <button
                                                    ...
                                                    className={`p-2 rounded-lg flex items-center justify-center transition-all ${split.method === method
                                        ...
                                        <div className="flex-1 relative flex items-center">
                                            ...
                                            <input
                                                ...
                                                className="w-full bg-transparent border-none pl-9 pr-3 py-2 text-[18px] font-bold text-gray-900 focus:outline-none placeholder:text-gray-300"
                                                placeholder="0.00"
                                                step="0.01"
                                            />
                                        </div>
                            ...
                            <button
                                ...
                                className={`w-full py-3.5 border-2 border-dashed rounded-xl font-bold flex items-center justify-center gap-2 transition-all ${balance > 0.01
                            ...
                            <div className="bg-gray-50 rounded-2xl p-5 border border-gray-100 shadow-sm relative overflow-hidden">
                                <div className={`absolute top-0 left-0 w-1.5 h-full ${isBalanced ? 'bg-green-500' : balance > 0 ? 'bg-[#ea580c]' : 'bg-red-500'
                                    }`} />
                                <div className="flex justify-between items-center pl-2">
                                    <span className="font-semibold text-gray-600">Balance Remaining</span>
                                    <span className={`text-[20px] font-bold flex items-center ${isBalanced ? 'text-green-600' : balance > 0 ? 'text-[#ea580c]' : 'text-red-500'
                                        }`}>
                        ...
                        <div className="p-6 border-t border-gray-100 bg-white">
                            <button
                                ...
                                disabled={!isBalanced}
                                className={`w-full flex items-center justify-center gap-2 font-bold py-4 rounded-xl transition-all h-[56px] text-[16px] shadow-sm ${isBalanced
                                    ? 'bg-green-600 hover:bg-green-700 text-white shadow-[0_8px_20px_-6px_rgba(22,163,74,0.4)] hover:-translate-y-0.5'
                                    : 'bg-gray-100 text-gray-400 cursor-not-allowed border border-gray-200'
                                    }`}
                            >
                                {isBalanced ? <CheckCircle2 size={20} strokeWidth={2.5} /> : null}
                ...
                {step === 'processing' && (
                    <div className="flex flex-col items-center justify-center py-12 animate-in fade-in duration-300">
                        <div className="w-20 h-20 border-4 border-gray-100 border-t-[#ea580c] rounded-full animate-spin mb-6"></div>
                ...
                {step === 'success' && (
                    <div className="flex flex-col items-center text-center p-8 animate-in zoom-in-95 duration-500">
